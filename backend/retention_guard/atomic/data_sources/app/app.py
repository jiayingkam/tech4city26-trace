...
def create_app() -> Flask:
    ...
    from .routes import data_sources_bp
    ...
    def _json_unhandled_error(error):
        if isinstance(error, HTTPException):
            return error
        app.logger.exception("Unhandled data_sources service error", exc_info=error)
    ...
    def get_swagger():
        ...
        swag["info"]["title"] = "Data Sources API"
    ...
    swaggerui_bp = get_swaggerui_blueprint(
        "/swagger-ui",
        "/swagger",
        config={"app_name": "Data Sources API"},
    )
